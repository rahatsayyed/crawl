...
const extractMainTextFromPage = async (url) => {
  let page;
  try {
    ...
    fs.writeFileSync(`./data/data${url}.txt`, $("body").text());
...
export const crawlWebsite = async (mainUrl) => {
  try {
    ...
    const results = await processInBatches(
      ...
      (pageUrl) => extractMainTextFromPage(pageUrl)
    );
...
crawlWebsite("https://www.hcltech.com/");
